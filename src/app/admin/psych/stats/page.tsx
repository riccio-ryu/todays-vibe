...
interface StatsData {
  totalUsage: number;
  topTest: TestRankItem | null;
  uniqueUsers: number;
  testCount: number;
  dailyData: { date: string; count: number }[];
  testRanking: TestRankItem[];
}
...
const ICON_BY_SLUG: Record<string, string> = Object.fromEntries(
  allPsychTests.map((t) => [t.slug, t.icon])
);
...
export default function AdminPsychStatsPage() {
  ...
  function buildQuery(extra?: string) {
    const base = isCustom
      ...
      : `period=${period}`;
    return `/api/admin/psych/stats?${base}${extra ? `&${extra}` : ""}`;
  }
  ...
  // 심테 드릴다운 조회
  function handleTestClick(slug: string) {
    ...
    fetch(buildQuery(`testSlug=${slug}`))
      .then((r) => r.json())
      .then(setDetail)
      .catch(console.error)
  ...
  const maxTest  = data?.testRanking[0]?.count ?? 1;

  return (
    <div className="p-4 md:p-8">
      {/* 헤더 */}
      <div className="mb-8">
        <h2 className="text-xl sm:text-2xl font-bold text-white">심테 통계</h2>
        <p className="text-white/40 text-sm mt-1">
          일별 · 심테별 이용 현황을 확인합니다. 로직 심테(무료·서버 미경유)는 집계에 포함되지 않습니다.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          { label: "총 AI 심테 이용", icon: "🤖", value: loading ? null : (data?.totalUsage ?? 0).toLocaleString(), unit: "회" },
          { label: "가장 인기 심테",  icon: "⭐", value: loading ? null : data?.topTest ? `${ICON_BY_SLUG[data.topTest.slug] ?? "🧠"} ${data.topTest.title}` : "—", unit: "" },
          { label: "이용 유저 수",    icon: "👤", value: loading ? null : (data?.uniqueUsers ?? 0).toLocaleString(), unit: "명" },
          { label: "이용된 심테 종류", icon: "🧠", value: loading ? null : (data?.testCount ?? 0).toLocaleString(), unit: "종" },
        ].map((s) => (
      ...
      <div className="rounded-xl border border-white/10 overflow-hidden">
        ...
        ) : !data?.testRanking.length ? (
          <div className="px-5 py-12 text-center text-white/20 text-sm">데이터가 없습니다</div>
        ) : (
          <div className="divide-y divide-white/5">
            {data.testRanking.map((item, idx) => (
              <div key={item.slug}>
                {/* 심테 행 */}
                <button
                  onClick={() => handleTestClick(item.slug)}
                ...
                >
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    <span className={`text-white/30 text-xs transition-transform ${
                      selectedSlug === item.slug ? "rotate-180" : ""
                    }`}>▼</span>
                  </div>
                </button>
