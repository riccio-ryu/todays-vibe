...
import { allPsychTests, getPsychTestBySlug } from "@/data/psych-tests";
import PsychTestQuiz from "@/components/psych/PsychTestQuiz";
...
export async function generateMetadata({
  ...
  if (!test) return {};
  const title = `${test.title} 심리 테스트 — ${test.questions.length}문항 AI 해석 | 오늘운`;
  return {
    title,
    description: test.summary,
    alternates: { canonical: `${BASE_URL}/psych-test/${slug}` },
...
export default async function PsychTestDetailPage({
  ...
  const jsonLd = {
    ...
    "@type": "Quiz",
    name: `${test.title} 심리 테스트`,
    description: test.summary,
    educationalAlignment: { "@type": "AlignmentObject", alignmentType: "assesses", targetName: "심리 성향" },
