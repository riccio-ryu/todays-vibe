...
### 🛠️ [admin] 심테 런칭 전 정비 — 수정/추가 목록

> **원칙: 운세와 심리는 완전 독립.** admin에서 도메인별로 따로 관리(운세 그룹 ↔ 심리 그룹). 심테는 운세 인프라(`menus`)에 의존하지 않고 자립. 조사·방향은 `docs/admin-psych-management.md` 참고.

**✅ 완료 (2026-07-19)**
- admin 심테 **운세형 관리** — `/admin/psych`에 노출·순서·**별 소모(cost)**·**AI 프롬프트 override**·**문항/결과 미리보기** (편집 모달). 심테별 cost 과금·프롬프트 override 연동.
- admin **3그룹 재편** — 사이드바·홈을 공통(대시보드·회원·AI사용량) / 운세(배치·메뉴관리·기록·통계·프롬프트) / 심리(메뉴관리)로 분리.
