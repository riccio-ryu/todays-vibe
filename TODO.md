...
### 11. [애드센스] 재심사 신청 + 승인 후 홈 광고 복원
- **내용**: 콘텐츠 확장·배포·색인 요청(9·10번)은 완료. 이제 2주 운영하며 색인을 확인한 뒤 애드센스 재심사를 신청한다. 승인되면 홈 화면 광고를 복원한다. **전체 대응 기록은 `docs/google-adsense.md` 참고.**
- **목적**: 거절 직후 반복 신청은 심사 이력에 불리함. 색인·트래픽이 쌓인 상태로 신청해야 통과 확률이 올라감.
- **현황 (2026-07-13 기준)**:
  - 콘텐츠 228 URL 배포·라이브 검증 완료 (타로 79 + 꿈해몽 101 + 별자리·띠 24 등)
  - Search Console에 `sitemap.xml` 제출 + `/tarot-cards`, `/dream-dictionary` 색인 요청 완료
  - 사이트맵 상태가 "가져올 수 없음"으로 표시 중 — **사이트 문제 아님** (200/application/xml/Googlebot 접근 전부 정상 확인). 제출 직후의 GSC 표시 문제이므로 **7/16까지 관찰**, 그때도 그대로면 삭제 후 재제출 1회
- **재심사 신청 전 체크리스트 (전부 충족 후 신청, 사용자가 직접)**:
  - [ ] **2026-07-27 이후** (배포일 + 2주)
  - [ ] GSC 사이트맵 상태 "성공" 전환 확인
  - [ ] GSC 색인 보고서에서 새 페이지 **최소 50개 이상 색인** (또는 구글에서 `site:todays-vibe.com/tarot-cards` 검색으로 확인)
