...
**준비 중 (5종):** 자미두수, 출생 차트, 기문둔갑, AI 관상, AI 수상

### 🧠 **심리 테스트 (7종)** — `docs/psych-tests-catalog.md` 기준

MBTI·성격·연애·유형을 몇 문항으로 알아보는 심리 테스트. 낮/해 테마로 운세(밤)와 구분됩니다.

**🔢 즉시 결과 (로직 계산 · 로그인/비용 없음):** 기본 MBTI, 연애 스타일, 동물 유형, 직장인 빌런, 결정 스타일, 관계 유형  
**🤖 AI 해석 (회원 전용):** 오늘의 에너지 진단, 숨겨진 감정 찾기

- 로직형은 선택지 가중치·투표를 클라이언트에서 합산해 즉시 결과 (MBTI 16유형 / 카테고리 4유형)
- `/psych` · `/psych/[slug]` (SSG) — 데이터 파일만 추가하면 목록·상세·사이트맵에 자동 연동
...
- 다크모드 기반 **신비로운 디자인**
- **낮/밤 스와이프** — 메인에서 좌우 드래그로 운세(밤)↔심리 테스트(낮) 전환, 배경까지 낮/밤 전환
